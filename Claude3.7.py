...
class ClaudeSonnetCodeAssistant:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-3-7-sonnet-20250219"):
        ...
        self.always_use_vim_style = True  # Whether to always use the VIM-style diff viewer
    ...
    async def handle_generate(self, file_path: str, prompt: str):
        try:
            ...
            if os.path.exists(full_path):
                ...
                console.print("[yellow]File already exists. Will generate a diff for changes.[/yellow]")
        ...
        try:
            ...
            if len(segments) == 1:
                ...
                if self.always_use_vim_style:
                    console.print("[cyan]Using VIM-style interactive diff reviewer for incremental patches...[/cyan]")
                    accepted, final_content = await self.interactive_diff_review(original_content, new_content, file_path)
                else:
                    # Ask if they want to use interactive diff viewer
                    use_interactive = input("Would you like to use the interactive diff viewer? (y/n): ").strip().lower() == 'y'
